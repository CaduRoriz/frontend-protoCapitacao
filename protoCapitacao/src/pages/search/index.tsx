        ...
        <h2>Realizar Busca</h2>
          ...
          <form className={styles.bodyForm} onSubmit={handleSubmit(onSubmit)}>
            <div className={styles.fields}>
              <label className={`${styles.label} ${styles.labelCnae}`} >Cnae <button onClick={handleOpenModal}><img className={styles.info} src="info.png"/></button></label>
              <select className={styles.selectForm} id="cnae" {...register("cnae")}>
                <option value="">Selecione</option>
                <>
                {cnaes.map((element, i) => {
                  return(
                    <option key={i} value={`${element.number}`}>{`${i+1} - ${element.number}`}</option>
                  );
                })}
                </>
              </select>
            ...
            <div className={styles.fields}>
              ...
              <select className={styles.selectForm} id="bairro" {...register("bairro")}>
                <option value="">Selecione</option>
                <>
                {bairros.map(element => {
                  return(
            ...
            <div className={`${styles.fields} ${styles.inputLimite}`}>
              <label className={`${styles.label} ${styles.labelMax}`}>Limite de lojas (máx:20)</label>
              <input className={`${styles.inputForm} `}type="number" max="20" min="1" id="limite" {...register("limite")} />
              <span className={`${styles.spanErro}`} id="erroInput">{errors.limite?.message}</span>
            </div>
